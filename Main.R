...
# General model
# employee type and satisfaction have a -ve association so it is dropped off a final model
...
# Stress level also have a direct effect towards wid

model_6 <- glm(work_induced~department + workplace_factor + stress_level, 
               data = hcw_model,
               family = binomial(logit))

summary(model_6)             # addition of stress level improves the model
round(exp(cbind(coef(model_6),confint(model_6))),3)
